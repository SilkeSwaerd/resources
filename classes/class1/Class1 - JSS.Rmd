...
p <- ggplot(data, aes(x = x)) +
  geom_point(aes(y = y_data), alpha = 0.6, color = 'purple') +  # Plotting the data points
  geom_line(aes(y = y_line), color = 'black') +
  labs(    
    x = 'Average recent growth in personal income (%)',
    y = "Incumbent party's vote share",
...
#fit a model to it to see what happens
...
x_values2 <- seq(0, 4, length.out = 1000)
y_values_line2 <- intercept + slope * x_values2
y_values_data2 <- y_values_line2 + rnorm(length(x_values2), mean = 0, sd = residual_sd2)

data2 <- data.frame(
  x = x_values, 
  y_line = y_values_line2,
  y_data = y_values_data2
)

plot2 <- ggplot(data2, aes(x=x)) +
  geom_point(aes(y = y_data), alpha = 0.7, color = "blue") +
  geom_line(aes(y = y_line), color = "black") +
  labs(title="attempting this on my own") +
  theme_minimal() +
  theme(legend.position = "none") +
  geom_smooth(aes(y=y_data), method = "lm", se = FALSE, color = "black")
plot2
...
M12b <- stan_glm(y_data ~ x_values, data = data2, refresh = 0)
M12b
```
...
female_names
#get the last letter of their names 
female_names <- female_names %>% 
  #create a new column with last letter 
